...
# Function part_get_resize2fs(num, length_sub)
part_get_resize2fs()
{
  # Return the partition size for resize2fs
  partition_size=$(${toybox} cat /proc/partitions \
                 | ${toybox} grep "p${1}" \
                 | ${toybox} tr -s ' ' \
                 | ${toybox} sed 's/\..*//' \
                 | ${toybox} cut -d ' ' -f 4);
  partition_block_size=$(($(${toybox} blockdev --getbsz ${mmc_block}p${1}) / 1024));
  ${toybox} echo $(( (${partition_size} - ${2} / 1024) / ${partition_block_size} ));
}
